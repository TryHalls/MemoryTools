return function(Context)
    return function(page, window)
        ...
        C.Button(page, "FLY TO BASE", function()
            Context.Controllers.TeleportController:FlyToBase()
        end, C.Theme.accent)
        C.Button(page, "LOCAL DEBUG TP", function()
            Context.Controllers.TeleportController:ToBase()
        end)
        C.Label(page, "Local teleport is retained for manual debugging only.", 28, "muted")
